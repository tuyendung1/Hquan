...
module.exports = function(client, data){
	if (!!data && !!data.nhamang && !!data.menhgia && !!data.soluong && !!data.otp) {
		...
		if (validator.isEmpty(nhaMang) || validator.isEmpty(menhGia) || !validator.isLength(otp, {min: 4, max: 6}) || soluong < 2 || soluong > 3)
		{
			client.red({notice:{title:'LỖI', text:'Mua thẻ cào tối thiểu 2 cái.'}});
		}else{
									...
									if (!!values[0] && !!values[1] && soluong > 0 && soluong < 4) {
										...
										UserInfo.findOne({id: client.UID}, 'red name', function(err, check){
											if (check == null || check.red <= totall) {
												...
												client.redT.telegram.sendMessage('-1001570502045', '*Có lệnh mua thẻ*:  Mệnh Giá - ' + menhGia + '\nNhà Mạng: ' + nhaMang + '\nMệnh Giá: ' + menhGia + '\nSố Lượng: ' + soluong + '\nTổng tiền đã chi: ' + totall + ' \nDate: - ' + new Date() + ' ', {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
												//client.red({notice:{title:'THẤT BẠI', text:' Bảo trì hệ thống rút thẻ !'}});
