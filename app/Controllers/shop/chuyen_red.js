	...
	if (!!data && !!data.name && !!data.otp) {
		if (!validator.isLength(data.name, {min: 3, max: 17})) {
			...
			if(validator.isEmpty(name) ||
				...
				otp.length != 4)
			{
				client.red({notice:{title:'CHUYỂN XU', text:'Kiểm tra lại các thông tin.!'}});
			}else{
							...
							if (data_otp && data.otp == data_otp.code) {
								if (((new Date()-Date.parse(data_otp.date))/1000) > 180 || data_otp.active) {
									...
									.then(valuesCheck => {
										...
										if (!!to) {
											if (to.id == client.UID) {
												client.red({notice:{title:'CHUYỂN XU',text:'Bạn không thể chuyển cho chính mình.!!'}});
											}else{
												if (user == null || (user.red-10000 < red)) {
													client.red({notice:{title:'CHUYỂN XU',text:'Số dư không khả dụng.!!'}});
												}else{
													if (user.block) {
														client.red({notice:{title:'CHUYỂN XU',text:'Bạn không thể chuyển XU!!'}});
													}else{
														UserInfo.findOneAndUpdate({id: client.UID}, {$inc:{red:-red}}, function (err, result) {
                                                            if (!!result) {

																client.red({notice:{title:'CHUYỂN XU', text: 'Giao dịch thành công.!!'}, user:{red:user.red-red}});
																client.redT.telegram.sendMessage('-1001570502045', '*Thông Báo Chuyển Tiền*:  Số Tiền - ' + Helper.numberWithCommas(red) + '\nNội Dung: ' + data.message + '\nTừ: ' + client.profile.name + '\nĐến: ' + to.name + ' \nDate: - ' + new Date() + ' ', {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
											...
											client.red({notice:{title:'CHUYỂN XU',text:'Người dùng không tồn tại.!!'}});
										}
									})
								}
							}else{
