...
let SieuXe_red  = require('../../../Models/SieuXe/SieuXe_red');
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			UserInfo.findOne({id:client.UID}, red ? 'red name':'xu name', function(err, user){
				if (client.SieuXe.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					client.red({SieuXe:{status:0, notice:'Bạn không đủ ' + (red ? 'XU':'XU') + ' để quay.!!'}});
				}else{
					...
					HU.findOne({game:'sieu', type:bet, red:red}, {}, function(err2, dataHu){
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											red && client.redT.sendInHome({pushnohu:{title:'sieuxe Casino', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'sieuxe Casino', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
							...
							if (red) {
								...
								SieuXe_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
