...
let RoyAl_red  = require('../../../Models/RoyAl/RoyAl_red');
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			UserInfo.findOne({id:client.UID}, red ? 'red name':'xu name', function(err, user){
				if (client.RoyAl.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					client.red({RoyAl:{status:0, notice:'Bạn không đủ ' + (red ? 'XU':'XU') + ' để quay.!!'}});
				}else{
					...
					HU.findOne({game:'roy', type:bet, red:red}, {}, function(err2, dataHu){
						...
						.then(result => {
							...
							if (red) {
								...
								RoyAl_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
