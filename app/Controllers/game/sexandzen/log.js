...
module.exports = function(client, data){
	if (!!data && !!data.page) {
		...
		if (page < 1) {
			...
			let kmess = 10;
			Sexandzen_red.countDocuments({name: client.profile.name}).exec(function(err, total){
				Sexandzen_red.find({name: client.profile.name}, 'id win bet kq time', {sort:{'_id':-1}, skip: (page-1)*kmess, limit: kmess}, function(err, result) {
