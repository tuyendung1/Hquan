		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				...
				if (client.Sexandzen.free === 0 && user.red < tongCuoc) {
					client.red({sexandzen:{status:0, notice:'Bạn không đủ R để quay.!!'}});
				}else{
					...
					HU.findOne({game:'sexandzen', type:bet}, {}, function(err2, dataHu){
						...
						*/
						...
						.then(result => {
							...
							Sexandzen_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
