			...
			result.forEach(function(line_win) {
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						Caoboi_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
						 io.sendInHome({pushnohu:{title:'Liên Minh Huyền Thoại', name:user.name, bet:okHu}}); 
						nohu = true;
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'Cao Bồi', users:user.name, bet:bet_win, status:2}}});
			} 
