...
let spin = function(io, user){
	...
	HU.findOne({game:'candy', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
			...
			result.forEach(function(line_win) {
				if (line_win.win == 6) {
					if (line_win.type === 5) {
						...
						Candy_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
			...
			//	Candy_red.create({'name':user.name, 'type':bet, 'win':bet_win, 'bet':tongCuoc, 'kq':result.length, 'line':result.length, 'time':new Date()});
			}
