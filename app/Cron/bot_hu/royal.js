	...
	HU.findOne({game:'roy', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
			...
			result.forEach(function(line_win) {
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						nohu = true;
						RoyAl_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
			...
			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'roy', users:user.name, bet:bet_win, status:2}}});
			} 
