		...
		.then(result => {
			...
			result.forEach(function(line_win) {
				if(!nohu && line_win.win == 10) {
					...
					//	nohu = true;
						SieuXe_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
			...
			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'sieu', users:user.name, bet:bet_win, status:2}}});
			} 
 		//	if (!nohu && bet_win >= tongCuoc*2.24) {
