...
let spin = function(io, user){
	...
	let nohu      = false;
	// tạo kết quả
	HU.findOne({game:'daohaitac', type:bet, red:true}, {}, function(err2, dataHu){
		...
		.then(result => {
			...
			result.forEach(function(line_win) {
				if (line_win.win == 6) {
					if (line_win.type === 5) {
						...
						if (!nohu) {
							...
							bet_win += okHu;
							HU.updateOne({game:'daohaitac', type:bet, red:true}, {$set:{name:'', bet:dataHu.min}}).exec();
						}else{
							okHu = Math.floor(dataHu.min-Math.ceil(dataHu.min*2/100));
							bet_win += okHu;
						}
						
						io.sendInHome({pushnohu:{title:'Đảo Hải Tặc', name:user.name, bet:okHu}});
						nohu = true;
						Daohaitac_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
					}
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'daohaitac', users:user.name, bet:bet_win, status:2}}});
			} 
		//	HU.updateOne({game:'sexandzen', type:bet, red:true}, {$inc:huUpdate}).exec();
			HU.updateOne({game:'daohaitac', type:bet, red:true}, {$inc:huUpdate}).exec();
