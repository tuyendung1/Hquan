			...
			result.forEach(function(line_win) {
				if (line_win.win == 6) {
					if (line_win.type === 5) {
						...
						nohu = true;
						Sexandzen_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
					}
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'sexandzen', users:user.name, bet:bet_win, status:2}}});
			} 
